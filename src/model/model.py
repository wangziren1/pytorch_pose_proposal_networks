...
def resnet18(net_state_dict=None):
...
def resnet50(net_state_dict=None):
    model = ResNet(Bottleneck, [3, 4, 6, 3])
    if net_state_dict is None:
        print('Loading resnet50 pretrain weight...')
        model.load_state_dict(torch.load('./model/pretrain_weight/resnet18-5c106cde.pth'))
    model.fc = nn.Sequential(nn.Conv2d(2048, 512, kernel_size=3, stride=1, padding=1),
                             nn.BatchNorm2d(512),
                             nn.LeakyReLU(negative_slope=0.1),
                             nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=1),
                             nn.LeakyReLU(negative_slope=0.1),
                             nn.Conv2d(512, 1311, kernel_size=1, stride=1, padding=0))
    if net_state_dict is not None:
        print('Loading resnet50 checkpoint weight...')
        model.load_state_dict(net_state_dict)
    return model


def which_model(is_shallow, net_state_dict=None):
    if is_shallow:
        return resnet18(net_state_dict)
    else:
        return resnet50(net_state_dict)
